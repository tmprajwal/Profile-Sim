...
double ranz(double zmin, double zmax);
...
double getnew(double rnoz, double xnoz, double ynoz, double znoz, double& x, double& y, double& z, double& vx, double& vy, double& vz, double& xmem, double& ymem, double& zmem, double ltube)	/* generates a new particle at(0,0,0) with random direction */
...
//	cout<<"three"<<" "<<phi<<endl;
	...
	z=ranz(0.0,ltube);			//"z" is the direction which the nozzle is facing. All particles start at the same location in z. MB
//	z=0;
...
//	rtube=rnoz;
	...
	rnoz=6.3;	rtube=6.3;	ltube=1295/2;	//ltube=1295*0.1;
	
	ofstream output;
	output.open("blocked_profile.txt");
	//Create variable to hold output file and create/open/rewrite an output file.

	ofstream putty;
	putty.open("blocked_initial_profile.txt");
	
	ofstream errorout;
	errorout.open("blocked_error_file.txt");
	
	ofstream track;
	track.open("blocked_tracker.txt");
	
	ofstream bout;
	bout.open("blocked_bounce.txt");
	...
	{
		avthetas+=getnew(rnoz,xnoz,ynoz,znoz,x,y,z,vx,vy,vz,xmem,ymem,zmem,ltube);
		newtheta(vx,vy,vz,x,y);
		bounce[i]=0;
		flag=0;
...
//*/				
			...
			if((z<=ztube+0.0000000000001 && bounce[i]>0) || (z<=ztube+0.0000000000001 && vz<0))
			{
				flag=0-2;
				kfail++;
				storenum++;
//				cout<<kfail<<"	"<<kpass<<"	"<<bounce[i]<<endl;
		...
		if(storenum<i+1)
		{
			cout<<kpass+kfail<<" "<<i+1<<":"<<endl<<"flag: "<<flag<<"	x: "<<x<<"	y: "<<y<<"	z: "<<z<<"	vx: "<<vx<<"	vy: "<<vy<<"	vz: "<<vz<<endl<<"	R: "<<sqrt(x*x+y*y)<<"	Bounce: "<<bounce[i]<<endl;
		}
