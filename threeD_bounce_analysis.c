...
void threeD_bounce_analysis()
{
	...
	ifstream input;
//	input.open("sc_profile_GhostRanch_detector_VLN.txt");
//	input.open("sc_profile_MCplate_VLN.txt");
//	input.open("sc_profile_LRratio_1_MN.txt");
//	input.open("sc_profile_GhostRanch_nozzle_VLN.txt");
	input.open("sc_profile_LRratio_50_100M.txt");

	while(!input.eof())
	{
		input>>num[polcount]>>xqstore[polcount]>>yqstore[polcount]>>zqstore[polcount]>>vx[polcount]>>vy[polcount]>>vz[polcount]>>theta[polcount]>>phi[polcount]>>bounce[polcount];

		theta[polcount]=theta[polcount]*180/pi;
		
		if(vx[polcount]<0)
		{theta[polcount]=-theta[polcount];}
		
		bounce_avg+=bounce[polcount];
		
		xstore[polcount]=bounce[polcount];
		ystore[polcount]=theta[polcount];
...
//	xblip=3;
	...
	yblip=(ymax-ymin)/ybins;
//	yblip=3;
	ybins=(ymax-ymin)/yblip;
	y1=ymin;
	y2=ymin+yblip;
/*
...
*/
	...
	for(int p=0;p<polcount;p++)
	{
		...
		for(int py=0;py<ybins;py++)
		{

			if(phi[p]>=(2*pi-0.024995) || phi[p]<=0.024995)
			{
				center1=(y1+y2)/2;

				if(ystore[p]>=y1 && ystore[p]<=y2)
				{
					ly=py*ybins;
...
//*/
	...
	gStyle->SetTitleSize(.08,"t");
		...
		ABS->cd(1);	
		TGraph2DErrors *gr0=new TGraph2DErrors(bins, ystore, xstore, xgram, 0, 0, 0);
...
*/
		gr0->SetTitle("Bounce Distribution (Single Channel); Angle (degrees); Number of Bounces; Number of Particles");
