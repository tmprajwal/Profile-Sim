...
//	Geometry, in mm. Filling in some values from start of the main.
	...
	zRGA=ztube+ltube+50.8;			//	My addition for the movefree function. Number indicates how far RGA is from end of quad. Currently just an estimate. MB
/*
	ztube=214.6;					//Actual Dimensions as measured on Device and in use 4/5/2021. Some of this is now very outdated.
	zskim=57;	rskim=3.4;
	zaper=170.23; raper=4.5;
	zRGA=ztube+ltube+50.8;
//*/	
//*
	zskim=57.45;	rskim=3.18;
	zaper=205.23;	raper=3.4;
	
	ztube=238.76;
	zaper=193.6;					//1/21/2021
	zRGA=ztube+ltube+50.8;			//Newer dimnensions. Measured to best of ability where possible. Still need better measurements for z-values.
//*/
...
/*
	ztube=214;						//ztube=200, raper=4.8, zskim=60, rskim=3.6
	zRGA=ztube+ltube+50.8;			//ztube=214, raper=4.8,
	
	zskim=165;	rskim=4.3;
	zaper=197.43; raper=4.8;
//*/
/*
	FILE *in;
	FILE *out;
...
//			cout<<"third"<<endl;
			...
			{
				v=ranmax();
				vx=v*sin(thetasc[numy])*cos(phisc[numy]);
				vy=v*sin(thetasc[numy])*sin(phisc[numy]);
				vz=v*cos(thetasc[numy]);
/*
				vx=vxsc[numy]/1000;
				vy=vysc[numy]/1000;
				vz=vzsc[numy]/1000;
...
//			putty<<endl<<x<<" "<<y<<" "<<z;
			...
			flag=moveskim(xskim,yskim,zskim,rskim,znoz,vx,vy,vz,x,y,z); //Ray Tracing
...
//				Move particle through quad. MB

				if(flag==-3)
				{
					zcheck=(z-ztube)/ltube;
					for(int i=0;i<ndiv;i++)
					{
						if(zcheck>=double(i)/double(ndiv) && zcheck<double(i+1)/double(ndiv))
						{section[i]++;}
					}
/*					
					if(zcheck>=0 && zcheck<0.1)
					{k10++;}
					if(zcheck>=0.1 && zcheck<0.2)
					{k20++;}
					if(zcheck>=0.2 && zcheck<0.3)
					{k30++;}
					if(zcheck>=0.3 && zcheck<0.4)
					{k40++;}
					if(zcheck>=0.4 && zcheck<0.5)
					{k50++;}
					if(zcheck>=0.5 && zcheck<0.6)
					{k60++;}
					if(zcheck>=0.6 && zcheck<0.7)
					{k70++;}
					if(zcheck>=0.7 && zcheck<0.8)
					{k80++;}
					if(zcheck>=0.8 && zcheck<0.9)
					{k90++;}
					if(zcheck>=0.9 && zcheck<=1)
					{k100++;}
//*/					
				}
...
/*
	cout<<endl<<"Percent lost in 0-10% of Quad: "<<(k10*100.0)/(khole)<<"%"<<endl<<"Percent lost in 10-20% of Quad: "<<k20*100.0/(khole)<<"%"<<endl;
	cout<<"Percent lost in 20-30% of Quad: "<<(k30*100.0)/(khole)<<"%"<<endl<<"Percent lost in 30-40% of Quad: "<<k40*100.0/(khole)<<"%"<<endl;
	cout<<"Percent lost in 40-50% of Quad: "<<(k50*100.0)/(khole)<<"%"<<endl<<"Percent lost in 50-60% of Quad: "<<k60*100.0/(khole)<<"%"<<endl;
	cout<<"Percent lost in 60-70% of Quad: "<<(k70*100.0)/(khole)<<"%"<<endl<<"Percent lost in 70-80% of Quad: "<<k80*100.0/(khole)<<"%"<<endl;
	cout<<"Percent lost in 80-90% of Quad: "<<(k90*100.0)/(khole)<<"%"<<endl<<"Percent lost in 90-100% of Quad: "<<k100*100.0/(khole)<<"%"<<endl;
//*/
/*	
	cout<<endl<<"Loop Method:"<<endl;
	for(int i=0;i<ndiv;i++)
	{
		cout<<"Percent lost in "<<i*100.0/ndiv<<"-"<<(i+1)*100.0/ndiv<<"% of Quad: "<<(2*i+1)*100.0/(2*ndiv)<<": "<<section[i]*100.0/khole<<"%"<<endl;
	}
//*/
	output.close();
