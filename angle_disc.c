...
//	input.open("sc_profile_LRratio_40_10M.txt");
//	input.open("initial_profile_mcplate_nozzle_radius1_toofast.txt");
//	input.open("initial_profile_mcplate1.txt");
//	input.open("initial_profile_1K_1.txt");

	while(!input.eof())
	{
		input>>num[polcount]>>xstore[polcount]>>ystore[polcount]>>zstore[polcount]>>vx[polcount]>>vy[polcount]>>vz[polcount]>>theta[polcount]>>phi[polcount]>>bounce[polcount];
...
//					{
						xgram[px]++;
//						xgram[px]=xgram[px]+1/(40*abs(sin(center1)));
						check++;
//					}
...
//	TCanvas *ABS= new TCanvas("ABS","Predicted Beam Profile",10,10,1280,620);
	TCanvas *ABS= new TCanvas("ABS","Predicted Beam Profile",10,10,620,620);
	ABS->SetGridx(0);
	ABS->SetGridy(0);
	ABS->SetFillColor(10);
...
//		gr0->SetTitle("Needle Nozzle Angular Velocity distribution (Single Channel)");
		gr0->SetTitle("Simulated Initial Profile New Nozzle");
		gr0->GetXaxis()->SetTitle("Angle (rads)");
