...
//out2<<x<<" "<<y<<" "<<z<<" "<<vx<<" "<<vy<<" "<<vz<<endl;
	...
	ifstream input;
	input.open("sc_tracker.txt");
	
	cout<<"goodbye"<<endl;

	while(!input.eof())
	{
		input>>particle[num]>>x[num]>>y[num]>>z[num];

		if(num>0)
		{
//			if(z[num]==0 && npol[num]==-1)
			if(particle[num]>particle[num-1])
			{
				length[n]=num-1-spot;
//				cout<<length[n]<<endl;
				n++;
...
//	cout<<check<<endl;
	...
	double gx[1000], gy[1000], gz[1000], gr[1000];
	...
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<length[i]-1;j++)
		{
			...
			gy[j]=yish[i][j];
			gz[j]=zish[i][j];
			gr[j]=sqrt(gx[j]*gx[j]+gy[j]*gy[j]);
		}

		gr1=new TGraphErrors(length[i]-1,gx,gz,0,0);
