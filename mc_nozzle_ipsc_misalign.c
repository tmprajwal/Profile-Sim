...
#define ntot 100000000		//Number of particles
...
*/
	...
	ofstream output;
//	output.open("profile_misalign.txt");
	...
	ofstream out2;
	out2.open("position_misalign.txt");
	
	ofstream steve;
	steve.open("exit_profile.txt");
	
	ofstream misalign ("misalign_table.txt", std::ofstream::out | std::ofstream::app);
...
//	xskim=xnoz;
	...
	misalign<<endl<<"ynoz (mm):	"<<ynoz<<endl<<"yskim (mm):	"<<yskim<<endl<<"beta (rads):	"<<endl;
	misalign<<"Percent lost on first skimmer: "<<endl<<"Percent through first skimmer: "<<endl;
	misalign<<"Percent lost on second skimmer: "<<endl<<"Percent through second skimmer: "<<endl;
	misalign<<"Percent lost to Quad entryway: "<<endl<<"Percent \"rejected\" by Quad field: "<<endl;
	misalign<<"Percent lost to Quad: "<<endl<<"Percent through Quad: "<<endl;
	...
	do{
			...
			if(save==0)
			{output.open("profile_misalign_beta0.txt");}
			
			if(save==1)
			{output.open("profile_misalign_beta1.txt");}
			
			if(save==2)
			{output.open("profile_misalign_beta2.txt");}
			
			if(save==3)
			{output.open("profile_misalign_beta5.txt");}
			
			if(save==4)
			{output.open("profile_misalign_beta10.txt");}
			
			if(save==5)
			{output.open("profile_misalign_beta15.txt");}
			
			if(save==6)
			{output.open("profile_misalign_beta20.txt");}
			
			if(save==7)
			{output.open("profile_misalign_beta25.txt");}
			
			if(save==8)
			{output.open("profile_misalign_beta30.txt");}
			
			if(save==9)
			{output.open("profile_misalign_beta40.txt");}
			
			if(save==10)
			{output.open("profile_misalign_beta50.txt");}
...
//				cout<<"getnew"<<endl;			
				...
				{
/*					//alpha
					vx=(cos(alpha)*vxsc[numy]/1000)-(sin(alpha)*vzsc[numy]/1000);
					vy=vysc[numy]/1000;
					vz=(sin(alpha)*vxsc[numy]/1000)+(cos(alpha)*vzsc[numy]/1000);
//*/
//*					//beta
					vx=vxsc[numy]/1000;
					vy=(cos(beta)*vysc[numy]+sin(beta)*vzsc[numy])/1000;
					vz=(0-sin(beta)*vysc[numy]+cos(beta)*vzsc[numy])/1000;
//*/
					thetay=atan2(vz,sqrt(vx*vx+vy*vy));
					numy++;
					if(numy>scl)
					{numy=0;}
...
//		Output average intial angle for particles. Helps for error checking and showing if the initial angle is artificially constrained for some other analysis. Artificially constraining the angle produces more particles that make it through the quad, ignoring the ones that do not. This allows for more statistics to be gathered on the output polarization by not wasting time on particles that are guaranteed to be lost anyway. The initial angle can be artificially constrained by changing thetamax in the getnew() function. MB
		...
		misalign<<beta<<endl;
		misalign<<kskim*100.0/ktot<<"%"<<endl<<(ktot-kskim)*100.0/ktot<<"%"<<endl;
		misalign<<kaper*100.0/ktot<<"%"<<endl<<(ktot-kskim-kaper)*100.0/ktot<<"%"<<endl;
		misalign<<kentry*100.0/ktot<<"%"<<endl<<khole*100.0/ktot<<"%"<<endl;
		misalign<<(kentry+khole)*100.0/ktot<<"%"<<endl<<(ktot-kskim-kaper-kentry-khole)*100.0/ktot<<"%"<<endl<<endl;
/*
		if(alpha==50e-3){alpha+=1;}
		if(alpha==40e-3){alpha=50e-3;}
		if(alpha==30e-3){alpha=40e-3;}
		if(alpha==25e-3){alpha=30e-3;}
		if(alpha==20e-3){alpha=25e-3;}
		if(alpha==15e-3){alpha=20e-3;}
		if(alpha==10e-3){alpha=15e-3;}
		if(alpha==5e-3){alpha=10e-3;}
		if(alpha==2e-3){alpha=5e-3;}
		if(alpha==1e-3){alpha=2e-3;}
		if(alpha==0){alpha=1e-3;}
//*/
//*		
		if(beta==50e-3){beta+=1;}
		if(beta==40e-3){beta=50e-3;}
		if(beta==30e-3){beta=40e-3;}
		if(beta==25e-3){beta=30e-3;}
		if(beta==20e-3){beta=25e-3;}
		if(beta==15e-3){beta=20e-3;}
		if(beta==10e-3){beta=15e-3;}
		if(beta==5e-3){beta=10e-3;}
		if(beta==2e-3){beta=5e-3;}
		if(beta==1e-3){beta=2e-3;}
		if(beta==0){beta=1e-3;}
//*/
		save++;
	}while(beta<=50e-3);
